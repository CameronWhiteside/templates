...
```typescript
{
  ...
  enabled?: boolean;            // Whether rule is active (optional, default: true)
}
```

**About `except_bots`:** Bot names are resolved to detection IDs at build time. Unknown names are skipped with a warning.
...
**Build process:** When you run `npm run deploy`, the build script automatically transforms `PRICING_RULES` from wrangler.jsonc into a dashboard-compatible `src/config.json`. This file is gitignored and regenerated on each deploy.
...
| File | Purpose |
|------|---------|
| `src/config.json` | Dashboard-compatible config (auto-generated by build script) |
