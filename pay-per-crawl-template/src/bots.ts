/**
 * Pay Per Crawl Template - Bot Name Registry
 *
 ...
 *
 * Users can specify bot names like "Googlebot" in their config,
 * and this module resolves them to detection IDs for matching.
 */
...
/**
 * Resolve bot names to detection IDs.
...
export function resolveBotsToIds(botNames: string[]): number[] {
	...
	for (const name of botNames) {
		...
		if (!bot) {
			// Unknown bot name - this should be caught during config validation
			console.warn(`[pay-per-crawl] Unknown bot name: "${name}". Skipping.`);
			continue;
		}

		// Prefer detection IDs, fall back to heuristics
		if (bot.detectionIds.length > 0) {
			ids.push(...bot.detectionIds);
		} else if (bot.heuristicsIds.length > 0) {
			ids.push(...bot.heuristicsIds);
		}
	}
...
/**
 * Get bot entry by user agent string (case-insensitive partial match).
 * Useful for suggesting bot names based on user agent.
 */
export function findBotByUserAgent(userAgent: string): BotEntry | null {
	const ua = userAgent.toLowerCase();

	for (const bot of Object.values(BOT_REGISTRY)) {
		if (bot.userAgent && ua.includes(bot.userAgent.toLowerCase())) {
			return bot;
		}
	}
