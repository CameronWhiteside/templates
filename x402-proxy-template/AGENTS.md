...
**With Bot Management Filtering:** If the user has Cloudflare Bot Management on their zone, x402-proxy can implement "default closed" - humans pass through free, only bots must pay. This is optional and enhances the base functionality.
...
|--------|---------------|----------|
| **Search engines only** | Googlebot, BingBot, Applebot | Maintain SEO visibility |
| **Search + AI assistants** | Above + ChatGPT-User, Claude-User, MistralAI-User, etc. | Allow content citations in AI answers |
...
**Example resolution:**
- User says: "Search engines only"
- Agent looks up: Googlebot → 120623194, BingBot → 117479730, Applebot → 120424214
- Config output:
```jsonc
...
```jsonc
{
  "routes": [{ "pattern": "example.com/premium/*", "zone_name": "example.com" }],
  "vars": {
    ...
    "PROTECTED_PATTERNS": [
      {
        ...
        // - Humans (bot score > 30) access for FREE
        // - Search engines (listed below) access for FREE
        // - All other bots (score <= 30) must pay $0.10
        // ─────────────────────────────────────────────────────────────────
...
When users have Bot Management enabled on their zone, they can configure payment exemptions based on bot score and detection IDs.
...
| Applebot | 120424214 | Apple Search / Siri |

#### AI Crawlers
| Bot Name | Detection ID | Notes |
|----------|--------------|-------|
| GPTBot | 123815556 | OpenAI training |
| CCBot | 133621792 | Common Crawl |
| Google-CloudVertexBot | 133730073 | Google AI training |
| Meta-ExternalAgent | 124581738 | Meta AI training |
| Amazonbot | 118601807 | Amazon AI |
| PetalBot | 124150991 | Huawei |
| Novellum AI Crawl | 121781688 | Novellum |
| Anchor Browser | 124259203 | Anchor AI browser |
...
| MistralAI-User | 128950951 | Mistral chat |
| DuckAssistBot | 126666910 | DuckDuckGo AI |
...
| Bot Name | Detection ID | Notes |
|----------|--------------|-------|
| archive.org_bot | 131190766 | Internet Archive |

#### Bots with Heuristics IDs Only

Some bots don't have verified detection IDs and use heuristics IDs instead:
...
| Bytespider | 33563853 | ByteDance / TikTok |
| FacebookBot | 33563972 | Meta crawling |
| Timpibot | 33564051 | Timpi search |

### Exception Presets

**Search Engines Only** (maintains SEO):
```jsonc
"except_detection_ids": [
  120623194,  // Googlebot - Google Search
  117479730,  // BingBot - Microsoft Bing
  120424214   // Applebot - Apple Search / Siri
]
```

**Search + AI Assistants** (allows real-time citations):
```jsonc
"except_detection_ids": [
  // Search Engines
  120623194,  // Googlebot - Google Search
  ...
  120424214,  // Applebot - Apple Search / Siri
  // AI Assistants
  132995013,  // ChatGPT-User - ChatGPT browsing mode
  33564303,   // Claude-User - Claude web access (heuristics)
  33564371,   // Perplexity-User - Perplexity answers (heuristics)
  128950951,  // MistralAI-User - Mistral chat
  126666910,  // DuckAssistBot - DuckDuckGo AI
  132272919   // Meta-ExternalFetcher - Meta AI assistant
]
```
...
**Causes:**
- Bot Management is not enabled on the zone
- Request is from local development (Bot Management not available locally)

**Fix:**
- Enable Bot Management in Cloudflare dashboard: Zone → Security → Bots → Configure Bot Management
...
3. Bot Management is enabled on the zone
...
- [ ] Bot Management is enabled on the zone
- [ ] `bot_score_threshold` is set on relevant patterns (typically 30)
- [ ] `except_detection_ids` are resolved from bot names with inline comments
